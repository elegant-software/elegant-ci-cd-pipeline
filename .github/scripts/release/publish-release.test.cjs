...
test("creates a missing tag and release at the planned target SHA", async () => {
  ...
  assert.equal(calls[1][1].prerelease, false);
});

test("marks release as prerelease when plan.prerelease is true", async () => {
  const calls = [];
  const github = { rest: {
    git: {
      getRef: async () => { throw Object.assign(new Error("not found"), { status: 404 }); },
  ...
  const { tmpdir } = require("node:os");
  const { join } = require("node:path");
  const workspace = mkdtempSync(join(tmpdir(), "publish-release-prerelease-"));
  mkdirSync(join(workspace, "release-output"));
  writeFileSync(join(workspace, "release-output/release-plan.json"), JSON.stringify({
    releaseRequired: true, nextVersion: "1.3.1-alpha", targetSha: "target-sha", prerelease: true,
  }));
  writeFileSync(join(workspace, "release-output/release-notes.md"), "## Features\n- alpha feature\n");
  process.chdir(workspace);
  try {
    await publishRelease({ github, context: context(), core: { info() {} } });
    assert.equal(calls[1][1].tag_name, "v1.3.1-alpha");
    assert.equal(calls[1][1].prerelease, true);
  } finally {
    process.chdir(originalDirectory);
    rmSync(workspace, { recursive: true, force: true });
  }
